...
.. code-block:: bash
    ...
    ./isaaclab.sh -p scripts/demos/sensors/ppisp_camera_nurec.py \
        --input_scene /path/to/scene.usd --renderer isaac_rtx --visualizer none --max_steps 60

    # Render multiple source-camera trajectory viewpoints in one tiled camera batch.
    ./isaaclab.sh -p scripts/demos/sensors/ppisp_camera_nurec.py \
        --input_scene /path/to/scene.usd --renderer isaac_rtx --visualizer none --num_viewpoints 4
...
parser.add_argument(
    ...
    default=None,
    help=(
        "Optional USD time codes to render as separate viewpoints. When set, the script bakes one "
        "source-camera pose per duplicated env and overrides --camera_time_code."
    ),
)
parser.add_argument(
    "--num_viewpoints",
    ...
    default=None,
    help=(
        "Sample this many source-camera trajectory time codes as separate viewpoints. Ignored when "
        "--camera_time_codes is provided."
    ),
)
parser.add_argument(
    "--num_envs",
    type=int,
    default=1,
    help="Number of duplicated input-scene envs to render when no explicit multi-view option is set.",
)
...
parser.add_argument(
    ...
    type=float,
    default=None,
    help="Override duplicated USD camera focalLength before creating Isaac Lab camera sensors.",
)
parser.add_argument(
    "--camera_horizontal_aperture",
    type=float,
    default=None,
    help="Override duplicated USD camera horizontalAperture before creating Isaac Lab camera sensors.",
)
parser.add_argument(
    "--camera_vertical_aperture",
    type=float,
    default=None,
    help="Override duplicated USD camera verticalAperture before creating Isaac Lab camera sensors.",
)
...
if args_cli.num_viewpoints is not None and args_cli.num_viewpoints < 1:
    parser.error("--num_viewpoints must be at least 1.")
...
if args_cli.camera_focal_length is not None and args_cli.camera_focal_length <= 0.0:
    parser.error("--camera_focal_length must be positive.")
if args_cli.camera_horizontal_aperture is not None and args_cli.camera_horizontal_aperture <= 0.0:
    parser.error("--camera_horizontal_aperture must be positive.")
if args_cli.camera_vertical_aperture is not None and args_cli.camera_vertical_aperture <= 0.0:
    parser.error("--camera_vertical_aperture must be positive.")
...
def enable_spg_runtime_settings() -> None:
    """Enable runtime SPG settings for NuRec particle_spg-runtime scenes."""
    import carb
    import omni.kit.app
    ...
    print("[INFO] Enabled NuRec runtime SPG settings.", flush=True)
...
def find_ppisp_camera_bindings(stage: Usd.Stage) -> list[tuple[str, Usd.Prim, Usd.Prim]]:
    ...
    for prim in stage.Traverse():
        ...
        if not shader_prim or not shader_prim.IsValid():
            shader_prim = stage.GetPrimAtPath(prim.GetPath().AppendChild("PPISPAuto"))
            if not shader_prim or not shader_prim.IsValid():
                continue
...
def collect_camera_xform_time_samples(source_stage: Usd.Stage, source_camera_prim_path: str) -> list[float]:
    """Collect authored xform time samples from the camera and its ancestors under the default prim."""
    default_prim = source_stage.GetDefaultPrim()
    if not default_prim:
        return []

    camera_prim = source_stage.GetPrimAtPath(source_camera_prim_path)
    samples = set()
    prim = camera_prim
    default_prim_path = default_prim.GetPath()
    while prim and prim.IsValid() and prim.GetPath().HasPrefix(default_prim_path):
        xformable = UsdGeom.Xformable(prim)
        for op in xformable.GetOrderedXformOps():
            try:
                samples.update(float(value) for value in op.GetAttr().GetTimeSamples())
            except Exception:
                pass
        if prim.GetPath() == default_prim_path:
            break
        prim = prim.GetParent()
    return sorted(samples)


def sample_evenly(values: list[float], count: int) -> list[float]:
    """Sample ``count`` values evenly from a sorted list, preserving endpoints."""
    if count == 1:
        return [values[0]]
...
def resolve_view_time_codes(source_stage: Usd.Stage, source_camera_prim_path: str) -> list[float]:
    """Resolve one source-camera USD time code per rendered viewpoint."""
    if args_cli.camera_time_codes:
        return [float(value) for value in args_cli.camera_time_codes]

    if args_cli.num_viewpoints is not None:
        samples = collect_camera_xform_time_samples(source_stage, source_camera_prim_path)
        if samples:
            time_codes = sample_evenly(samples, args_cli.num_viewpoints)
            print(
                "[INFO] Sampled source camera xform time codes: "
                + ", ".join(f"{value:g}" for value in time_codes),
                flush=True,
            )
            return time_codes

        start_time = float(source_stage.GetStartTimeCode())
        end_time = float(source_stage.GetEndTimeCode())
        if end_time > start_time:
            time_codes = np.linspace(start_time, end_time, args_cli.num_viewpoints).tolist()
            print(
                "[INFO] No xform samples found; sampled stage time range: "
                + ", ".join(f"{value:g}" for value in time_codes),
                flush=True,
            )
            return time_codes
        ...
        print(
            "[WARNING] No camera trajectory time samples or stage time range found; "
            "rendering repeated copies of --camera_time_code.",
            flush=True,
        )
        return [float(args_cli.camera_time_code)] * args_cli.num_viewpoints

    return [float(args_cli.camera_time_code)] * args_cli.num_envs


def bake_source_camera_poses_to_envs(
    source_stage: Usd.Stage, source_camera_prim_path: str, view_time_codes: list[float]
) -> None:
    """Bake one selected USD camera pose into each duplicated env camera prim."""
    default_prim = source_stage.GetDefaultPrim()
    if not default_prim:
        raise RuntimeError("Input scene must have a defaultPrim so it can be referenced under each env.")
    ...
    for env_id in range(args_cli.num_envs):
        time_code_value = view_time_codes[env_id]
        time_code = Usd.TimeCode(time_code_value)
    ...
    print(
        "[INFO] Baked source camera poses into "
        f"{authored_count} env camera(s): "
        + ", ".join(f"env_{idx}=t{time_code:g}" for idx, time_code in enumerate(view_time_codes)),
        flush=True,
    )


def override_env_camera_intrinsics(source_stage: Usd.Stage, source_camera_prim_path: str) -> None:
    """Override duplicated camera USD intrinsics before Isaac Lab sensors read them."""
    overrides = {
        "focalLength": args_cli.camera_focal_length,
        "horizontalAperture": args_cli.camera_horizontal_aperture,
        "verticalAperture": args_cli.camera_vertical_aperture,
    }
    ...
    for env_id in range(args_cli.num_envs):
        ...
        if not target_camera_prim or not target_camera_prim.IsValid():
            raise RuntimeError(f"Duplicated camera prim not found: {target_camera_path}")
        if target_camera_prim.GetTypeName() != "Camera":
            raise RuntimeError(f"Duplicated prim is not a Camera: {target_camera_path}")

        camera = UsdGeom.Camera(target_camera_prim)
        before = {
            "focalLength": camera.GetFocalLengthAttr().Get(),
            "horizontalAperture": camera.GetHorizontalApertureAttr().Get(),
            "verticalAperture": camera.GetVerticalApertureAttr().Get(),
        }
        if args_cli.camera_focal_length is not None:
            camera.CreateFocalLengthAttr(float(args_cli.camera_focal_length)).Set(float(args_cli.camera_focal_length))
        if args_cli.camera_horizontal_aperture is not None:
            camera.CreateHorizontalApertureAttr(float(args_cli.camera_horizontal_aperture)).Set(
                float(args_cli.camera_horizontal_aperture)
            )
        if args_cli.camera_vertical_aperture is not None:
            camera.CreateVerticalApertureAttr(float(args_cli.camera_vertical_aperture)).Set(
                float(args_cli.camera_vertical_aperture)
            )
        after = {
            "focalLength": camera.GetFocalLengthAttr().Get(),
            "horizontalAperture": camera.GetHorizontalApertureAttr().Get(),
    ...
    print(
        "[INFO] Overrode camera intrinsics on "
        f"{authored_count} env camera(s): before={first_before}, after={first_after}",
        flush=True,
    )
...
def format_time_code_for_filename(time_code: float) -> str:
    """Format a USD time code for a filename."""
    return f"{time_code:g}".replace("-", "m").replace(".", "p")
    ...
    while simulation_app.is_running():
        ...
        if count % args_cli.save_interval == 0:
            ...
            for env_id in range(ppisp.shape[0]):
                view_label = f"env {env_id} t={view_time_codes[env_id]:g}"
                images.extend(
                    [
                ...
                subtitles.extend([f"{view_label} baseline", f"{view_label} PPISP", f"{view_label} diff"])
                time_label = format_time_code_for_filename(view_time_codes[env_id])
                save_tensor_image(
                    baseline[env_id].float() / 255.0,
                    os.path.join(output_dir, f"ppisp_camera_{count:04d}_env{env_id:02d}_t{time_label}_baseline.png"),
                )
                save_tensor_image(
                    ppisp[env_id].float() / 255.0,
                    os.path.join(output_dir, f"ppisp_camera_{count:04d}_env{env_id:02d}_t{time_label}_ppisp.png"),
                )
                save_tensor_image(
                    diff[env_id],
                    os.path.join(output_dir, f"ppisp_camera_{count:04d}_env{env_id:02d}_t{time_label}_diff.png"),
                )
            save_images_grid(
                images,
                nrow=ppisp.shape[0],
...
def main() -> None:
    ...
    view_time_codes = resolve_view_time_codes(source_stage, source_camera_prim_path)
    if len(view_time_codes) != args_cli.num_envs:
        print(
            f"[INFO] Adjusting --num_envs from {args_cli.num_envs} to {len(view_time_codes)} "
            "so each viewpoint maps to one duplicated env.",
            flush=True,
        )
        args_cli.num_envs = len(view_time_codes)
    ...
    bake_source_camera_poses_to_envs(source_stage, source_camera_prim_path, view_time_codes)
    override_env_camera_intrinsics(source_stage, source_camera_prim_path)
    ...
    print(
        ...
        f"at {len(view_time_codes)} viewpoint(s).",
        flush=True,
    )

    sim.reset()
    print("[INFO]: Setup complete. Saving comparison images during simulation.", flush=True)
    run_simulator(sim, baseline_camera, ppisp_camera, view_time_codes)
    del scene
